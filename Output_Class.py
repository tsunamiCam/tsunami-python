...
#                    i+=1
    ...
    def set_output_at_buildings_in_database(self):

        '''
        Set the timeseries output at buildings in the database
        '''
        ...
        for building in self.buildings_dict.iteritems():
            ...
            self.grid_pg.cur.execute("UPDATE buildings_run2000 SET eta_timeseries = ARRAY%s WHERE id = %s;" % (etaMax_dict['nodes'],id))
            self.grid_pg.cur.execute("UPDATE buildings_run2000 SET time = ARRAY%s WHERE id = %s;" % (time.tolist(),id))
...
#    def create_fragility_cumfreq2_norm(self, damage_class, mu1=1,sig1=0.4, mu2=2.5,sig2=0.6, mu3=3.25,sig3=0.6, c1=100, c2=100, c3=100):
    def create_fragility_cumfreq2_norm(self, damage_class,s1,s2 ):
...
#        #plt.close()
    ...
    def create_fragility_cumfreq_no_bin_norm(self, damage_class, s1,s2):
        '''
        Create a cumulative frequency (i.e. method used in valencia) fragility function using a set of
        normal distributed buildings
...
#
  ...
  #Compare with Suppasri2012a - Exclude washed away buildings
    ...
    def create_fragility_probit_ln_NEW(self,tablename="buildings_results_all_6march2016", column = "r1",where="",weight = True,log_depth=True):

        self.grid_pg.cur.execute("SELECT id,%s,D5 FROM %s" % (column,tablename))
        damaged_bldgs = self.grid_pg.cur.fetchall()
        depth = []
        phiinv = []
        histogram = []
    ...
    def get_tib_fdmax_ALL(self):
        ...
        if self.output_tablename == "":
            ...
            t = self.output_tablename
            self.grid_pg.cur.execute("SELECT id,fdmax_nodes FROM %s" % (t))
            query = self.grid_pg.cur.fetchall()
